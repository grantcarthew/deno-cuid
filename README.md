...
The `cuid()` function call returns a short random string with some collision-busting measures. Safe to use as HTML element ID's, and unique server-side record lookups.

This version of `cuid` is a conversion to [Deno](https://deno.land/) from the [original Node.js version](https://github.com/ericelliott/cuid) written by [Eric Elliott](https://github.com/ericelliott).
...
**In deno**, the first two chars are extracted from the process.pid. The next two chars are extracted from the hostname.
